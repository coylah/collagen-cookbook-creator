...
import { DAYS, SLOTS, planKey, useMealPlan, type PlanEntry } from "@/lib/user-state";
        ...
        <div className="mx-auto max-w-6xl px-4 py-8 sm:px-6">
          <div className="flex flex-wrap items-end justify-between gap-4">
            ...
            <div className="flex items-center gap-2">
              <Button variant="outline" size="sm" onClick={clear} className="text-muted-foreground">
      ...
      <section className="mx-auto max-w-6xl px-4 py-6 sm:px-6">
        <div className="w-full overflow-x-auto">
          <div className="min-w-[600px] grid grid-cols-[56px_repeat(4,1fr)] gap-1.5">
            <div />
            {SLOTS.map((s) => (
              <div key={s} className={`rounded-lg border px-2 py-2 text-center text-[10px] font-semibold uppercase tracking-wider capitalize ${SLOT_HEADER[s]}`}>
                {s}
              </div>
            ))}
        ...
        <p className="mt-4 text-xs text-muted-foreground text-center">
          Tap any slot to add a recipe · Glow Bowls and Yoghurt Bowls you've built also show here · Tap × to remove
        </p>
      </section>
    </AppShell>
  );
  ...
  return (
    <>
      ...
      {SLOTS.map((s) => {
        ...
        return (
          <div key={s} className={`min-h-[80px] rounded-lg border text-xs transition-all duration-150 ${cls}`}>
            {isCustom && entry ? (
